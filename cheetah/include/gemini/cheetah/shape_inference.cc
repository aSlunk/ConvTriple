...
std::optional<TensorShape> Conv2D(const TensorShape& tshape, const TensorShape& fshape,
...
bool Conv2D(const TensorShape& tensor_shape, const TensorShape& filter_shape,
    ...
    for (int d = 0; d < filter_shape.dims(); ++d) {
        if (tshape.dim_size(d) < filter_shape.dim_size(d)) {
            LOG(WARNING) << "Conv2D: tensor_shape.dim_size(d) < filter_shape.dim_size(d)";
            return false;
        }
    }
    ...
    if (HW <= poly_degree) {
        ...
        slice_strides[1] = std::min(static_cast<int>(std::sqrt(poly_degree * ratio)), H);
