...
class HomBNSS {
  public:
    ...
    Code encodeVector(const Tensor<uint64_t>& in_vec, const Meta& meta,
                      std::vector<seal::Plaintext>& out, size_t nthreads = 1) const;
    ...
    Code decryptToTensor(const std::vector<seal::Ciphertext>& in_vec, const Meta& meta,
  ...
  protected:
    ...
    Code addMaskPrimeField(std::vector<seal::Ciphertext>& ct, Tensor<uint64_t>& mask,
                           const Meta& meta, ThreadPool&) const;
    Code addMaskRing(std::vector<seal::Ciphertext>& ct, Tensor<uint64_t>& mask, const Meta& meta,
                     ThreadPool&) const;
    Code addMask(std::vector<seal::Ciphertext>& ct, Tensor<uint64_t>& mask, const Meta& meta,
                 ThreadPool&) const;
