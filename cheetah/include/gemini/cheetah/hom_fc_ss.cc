...
int64_t HomFCSS::poly_degree() const {
    if (context_) {
...
Code HomFCSS::setUp(const seal::SEALContext& context, std::optional<seal::SecretKey> sk,
...
Code HomFCSS::initPtx(seal::Plaintext& pt, seal::parms_id_type pid) const {
    ...
    const size_t num_elt    = seal::util::mul_safe(static_cast<int64_t>(num_moduli), poly_degree());
    pt.parms_id()           = seal::parms_id_zero; // foo SEAL when using BFV
    pt.resize(num_elt);
    pt.parms_id() = pid;
    ENSURE_OR_RETURN(pt.data() != nullptr, Code::ERR_SEAL_MEMORY);
...
Code HomFCSS::vec2PolyBFV(const uint64_t* vec, size_t len, seal::Plaintext& pt,
    ...
    ENSURE_OR_RETURN(len > 0 && len <= static_cast<size_t>(poly_degree()), Code::ERR_OUT_BOUND);
...
Code HomFCSS::vec2Poly(const uint64_t* vec, size_t len, seal::Plaintext& pt, const Role role,
                       bool is_to_ntt) const {
    switch (scheme()) {
...
Code HomFCSS::encodeInputVector(const Tensor<uint64_t>& input_vector, const Meta& meta,
    ...
    auto encode_prg = [&](long wid, size_t start, size_t end) {
        ...
        for (size_t i = start; i < end && !is_failed; ++i) {
            ...
            if (Code::OK
                != vec2Poly(tmp.data(), tmp.size(), encoded_share.at(i), encode_role, false)) {
                is_failed = true;
            }
        }
        seal::util::seal_memzero(tmp.data(), sizeof(uint64_t) * tmp.size());
...
Code HomFCSS::encodeWeightMatrix(const Tensor<uint64_t>& weight_matrix, const Meta& meta,
                                 std::vector<std::vector<seal::Plaintext>>& encoded_share,
                                 size_t nthreads) const {
    ENSURE_OR_RETURN(context_, Code::ERR_CONFIG);
    ENSURE_OR_RETURN(weight_matrix.shape().IsSameSize(meta.weight_shape), Code::ERR_DIM_MISMATCH);
    ...
    auto encode_prg = [&](long wid, size_t start, size_t end) {
        ...
        for (size_t r_blk = start; r_blk < end && !is_failed; ++r_blk) {
            ...
            auto top_right_row = std::min<size_t>(top_left_row + split_shape.rows(), nrows);
            ...
            for (size_t c_blk = 0; c_blk < n_col_blks; ++c_blk) {
                ...
                auto col_extent
                    = std::min<size_t>(top_left_col + split_shape.cols(), ncols) - top_left_col;
                // Encode the sub-matrix start ad (top_left_row, top_left_col) with
                // sizes (row_extent, col_extent) Matrix is stored in row-major
                auto src_ptr = weight_matrix.data() + top_left_row * ncols + top_left_col;
                ...
                if (Code::OK
                    != vec2Poly(tmp.data(), tmp.size(), encoded_share.at(r_blk).at(c_blk),
                                Role::none, false)) {
                    is_failed = true;
                    break;
                }
...
Code HomFCSS::MatVecMul(const std::vector<seal::Ciphertext>& vec_share0,
    ...
    auto fma_prg = [&](long wid, size_t start, size_t end) {
        for (size_t j = start; j < end; ++j) {
            evaluator_->multiply_plain(meta.is_shared_input ? input[0] : vec_share0[0],
                                       matrix[j][0], out_share0[j]);
            // TODO(wen-jie): to implement FMA
            for (size_t i = 1; i < n_ct_in; ++i) {
                seal::Ciphertext tmp;
                evaluator_->multiply_plain(meta.is_shared_input ? input[i] : vec_share0[i],
                                           matrix[j][i], tmp);
...
// coefficients.
Code HomFCSS::removeUnusedCoeffs(std::vector<seal::Ciphertext>& cts, const Meta& meta,
    ...
    for (size_t r_blk = 0; r_blk < n_ct_out; ++r_blk) {
        ...
        auto row_bgn  = r_blk * split_shape.rows();
        auto row_end  = std::min<size_t>(row_bgn + split_shape.rows(), meta.weight_shape.rows());
        long upper    = (row_end - row_bgn) * split_shape.cols();

        for (long index = 0; index < poly_degree(); ++index) {
...
Code HomFCSS::sampleRandomMask(const std::vector<size_t>& targets, uint64_t* coeffs_buff,
...
Code HomFCSS::decryptToVector(const std::vector<seal::Ciphertext>& enc_vector, const Meta& meta,
                              Tensor<uint64_t>& out, size_t nthreads) const {
