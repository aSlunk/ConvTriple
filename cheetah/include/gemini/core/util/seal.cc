...
static Code divide_and_round_q_last_ntt_inplace(seal::util::RNSIter input, size_t input_base_size,
                                                seal::util::ConstNTTTablesIter rns_ntt_tables,
                                                const seal::util::RNSTool* rns_tool,
    ...
    add_poly_scalar_coeffmod(last_input, coeff_count, half, last_modulus, last_input);

    SEAL_ALLOCATE_GET_COEFF_ITER(temp, coeff_count, pool);
    SEAL_ITERATE(iter(input, rns_tool->inv_q_last_mod_q(), base_q->base(), rns_ntt_tables),
        ...
        SEAL_ITERATE(temp, coeff_count, [&](auto &J) {
            J -= (qi_lazy & static_cast<uint64_t>(-static_cast<int64_t>(J >= qi_lazy)));
                     ...
                     multiply_poly_scalar_coeffmod(get<0>(I), coeff_count, get<1>(I), get<2>(I),
                                                   get<0>(I));
                 });
    return Code::OK;
}

static Code switch_key_inplace(seal::Ciphertext& encrypted, seal::util::ConstRNSIter target_iter,
    ...
    for (auto& each_key : key_vector) {
        if (!is_metadata_valid_for(each_key, context) || !is_buffer_valid(each_key)) {
            return Code::ERR_INVALID_ARG;
    ...
    SEAL_ITERATE(iter(size_t(0)), rns_modulus_size, [&](auto I) {
        ...
        SEAL_ITERATE(iter(size_t(0)), decomp_modulus_size, [&](auto J) {
            ...
            SEAL_ITERATE(
                iter(key_vector[J].data(), accumulator_iter), key_component_count, [&](auto K) {
                    if (!lazy_reduction_counter) {
                        SEAL_ITERATE(iter(t_operand, get<0>(K)[key_index], get<1>(K)), coeff_count,
                                     [&](auto L) {
        ...
        SEAL_ITERATE(iter(accumulator_iter, t_poly_prod_iter), key_component_count, [&](auto K) {
            if (lazy_reduction_counter == lazy_reduction_summand_bound) {
                SEAL_ITERATE(iter(get<0>(K), *get<1>(K)), coeff_count,
                             [&](auto L) { get<1>(L) = static_cast<uint64_t>(*get<0>(L)); });
            } else {
    ...
    SEAL_ITERATE(iter(encrypted, t_poly_prod_iter), key_component_count, [&](auto I) {
        // ntt form -> ntt form
        divide_and_round_q_last_ntt_inplace(get<1>(I), decomp_modulus_size, key_ntt_tables,
                                            rns_tool, pool);
        if (!is_ntt_form) {
            inverse_ntt_negacyclic_harvey(get<1>(I), decomp_modulus_size, key_ntt_tables);
        }
        add_poly_coeffmod(get<0>(I), get<1>(I), decomp_modulus_size, key_modulus, get<0>(I));
    });
    return Code::OK;
}
...
Code apply_galois_inplace(::seal::Ciphertext& encrypted, uint32_t galois_elt,
    ...
    // Verify parameters.
    if (!is_metadata_valid_for(encrypted, context) || !is_buffer_valid(encrypted)) {
        std::cerr << "encrypted is not valid for encryption parameters" << std::endl;
        return Code::ERR_INVALID_ARG;
    }

    // Don't validate all of galois_keys but just check the parms_id.
    if (galois_keys.parms_id() != context.key_parms_id()) {
        std::cerr << "galois_keys is not valid for encryption parameters" << std::endl;
        return Code::ERR_INVALID_ARG;
    }
    ...
    if (std::any_of(*temp, *temp + coeff_modulus_size * coeff_count,
        ...
        ok = switch_key_inplace(encrypted, temp, static_cast<const KSwitchKeys&>(galois_keys),
