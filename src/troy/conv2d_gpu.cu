...
void conv2d(IO::NetIO** ios, int party, INT_TYPE* a, INT_TYPE* b, INT_TYPE* c, size_t bs, size_t ic,
    ...
    void conv2d_dummy(IO::NetIO * *ios, int party, size_t bs, size_t ic, size_t ih, size_t iw,
                      size_t kh, size_t kw, size_t oc, size_t stride, size_t padding,
                      bool mod_switch) {
        ...
        conv2d(ios, party, x.data(), w.data(), c.data(), bs, ic, ih, iw, kh, kw, oc, stride,
               padding, mod_switch);
    }
    ...
    void conv2d_ab2(IO::NetIO * *ios, int party, INT_TYPE* x, INT_TYPE* w, INT_TYPE* c, size_t bs,
        ...
        if (party == 1) {
            ...
            linear::Plain2d w_encoded
                = helper.encode_weights_ring2k(encoder, w, std::nullopt, false);
    ...
    void conv2d_ab(IO::NetIO * *ios, int party, INT_TYPE* x, INT_TYPE* w, INT_TYPE* c, size_t bs,
        ...
        if (party == 1) {
            ...
            linear::Plain2d w_encoded
                = helper.encode_weights_ring2k(encoder, w, std::nullopt, false);
            linear::Plain2d R1_encoded
                = helper.encode_outputs_ring2k(encoder, R1.data(), std::nullopt);
            ...
            linear::Plain2d w_encoded
                = helper.encode_weights_ring2k(encoder, w, std::nullopt, false);
            linear::Plain2d R2_encoded
                = helper.encode_outputs_ring2k(encoder, R2.data(), std::nullopt);
    ...
    vector<INT_TYPE> ideal_conv(INT_TYPE * x, INT_TYPE * w, size_t t, size_t bs, size_t ic,
    ...
    void conv2d_ab2_reverse(IO::NetIO * *ios, int party, INT_TYPE* x, INT_TYPE* w, INT_TYPE* c,
                            size_t bs, size_t ic, size_t ih, size_t iw, size_t kh, size_t kw,
                            size_t oc, size_t stride, bool mod_switch) {
        using namespace troy;
        auto he = setup();
        linear::PolynomialEncoderRing2k<INT_TYPE> encoder(he, BIT_LEN);
        if (utils::device_count() > 0) {
        ...
        if (party == 2) {
            ...
            linear::Plain2d x_encoded
                = helper.encode_inputs_ring2k(encoder, x, std::nullopt, false);
            linear::Plain2d R_encoded
                = helper.encode_outputs_ring2k(encoder, R.data(), std::nullopt);
