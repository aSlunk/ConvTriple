...
struct ConvIndexer {
  public:
    explicit ConvIndexer(long N, TensorShape ishape, TensorShape fshape) {
        if (!ishape.IsValid() || !fshape.IsValid() || ishape.dims() != 3 || fshape.dims() != 3
            || ishape.dim_size(0) != fshape.dim_size(0)) {
...
struct ImageIndexer : public ConvIndexer {
  public:
    ...
    inline long operator()(long chl, long row, long col) const {
        ...
        if (chl >= shape_.dim_size(0) || row >= shape_.dim_size(1) || col >= shape_.dim_size(2)) {
...
struct FilterIndexer : public ConvIndexer {
  public:
    FilterIndexer(long poly_degree, TensorShape ishape, TensorShape fshape,
        ...
        // O = HW*(C-1) + W*(h-1) + (h-1)
        begin_ = offset_ * (fshape.channels() - 1) + ishape.width() * (h - 1) + h - 1;
    }
...
Code TensorEncoder::EncodeFilter(const U64Tensor& filter, const TensorShape& img_shape,
                                 const Padding& padding, const size_t stride,
                                 const bool is_ntt_form, std::vector<RLWEPt>& out) const {
    TensorShape fshape = filter.shape();
    ENSURE_OR_RETURN(img_shape.dims() == 3 && fshape.dims() == 3, Code::ERR_DIM_MISMATCH);
    if (static_cast<size_t>(filter.width() * filter.height()) > poly_degree()) {
    ...
    if (!shape_inference::Conv2D(img_shape, fshape, poly_degree(), padding, stride, strided_tshape,
                                 paddings, slice_strides)) {
    ...
    for (int c = 0; c < helper.num_slices(); ++c) {
        ...
        CHECK_ERR(Encode(sliced_ishape, sliced_fshape, sliced_filter, indexer, tmp_buf.get(),
                         poly_degree()),
                  "Encode");
        CHECK_ERR(A2H(tmp_buf.get(), poly_degree(), out.at(c), Role::evaluator, is_ntt_form),
                  "A2H");
    }
...
Code TensorEncoder::EncodeImageShare(Role role, const U64Tensor& img_tensor,
                                     ...
                                     const size_t stride, const bool is_ntt_form,
                                     std::vector<RLWEPt>& out) const {
    ENSURE_OR_RETURN(img_tensor.dims() == 3 && filter_shape.dims() == 3, Code::ERR_DIM_MISMATCH);

    TensorShape strided_tshape;
    std::array<int, 2> paddings{0};
    std::array<int, 3> slice_strides{0};
    if (!shape_inference::Conv2D(img_tensor.shape(), filter_shape, poly_degree(), padding, stride,
                                 strided_tshape, paddings, slice_strides)) {
    ...
    Conv2DSliceHelper helper(&strided_img, strided_img.shape(), filter_shape, slice_strides,
                             paddings);
    ...
    for (size_t i = 0; i < n_slices; ++i) {
        ...
        CHECK_ERR(
            Encode(si_shape, sf_shape, sliced_padded_img, indexer, tmp_buf.get(), poly_degree()),
            "Encode");

        CHECK_ERR(A2H(tmp_buf.get(), poly_degree(), out[i], role, is_ntt_form), "A2H");
    }
    return Code::OK;
}

template <class TensorType, class Indexer>
Code TensorEncoder::Encode(TensorShape ishape, TensorShape fshape, const TensorType& tensor,
                           const Indexer& indexer, U64* out_poly, size_t out_max_sze) const {
    ...
    ENSURE_OR_RETURN(fshape.channels() == ishape.channels(), Code::ERR_DIM_MISMATCH);
    ENSURE_OR_RETURN(tensor.shape().IsSameSize(ishape) || tensor.shape().IsSameSize(fshape),
    ...
    for (int c = 0; c < shape.channels(); ++c) {
        for (int h = 0; h < shape.height(); ++h) {
            for (int w = 0; w < shape.width(); ++w) {
                ...
                if (coeff_index < 0 || coeff_index >= static_cast<int64_t>(N)) {
...
Code TensorEncoder::A2HBFV(const U64* vec, size_t len, RLWEPt& pt, const Role role,
...
ConvCoeffIndexCalculator::ConvCoeffIndexCalculator(size_t poly_degree, TensorShape ishape,
                                                   TensorShape fshape, Padding padding,
                                                   size_t stride)
    : poly_degree_(poly_degree), stride_(stride), ishape_(ishape), fshape_(fshape) {
    if (!ishape.IsValid() || !fshape.IsValid())
        LOG(FATAL) << "invalid shape";

    TensorShape strided_ishape;
    std::array<int, 2> paddings;
    std::array<int, 3> slice_strides;
    if (!shape_inference::Conv2D(ishape, fshape, poly_degree, padding, stride, strided_ishape,
                                 paddings, slice_strides)) {
        LOG(FATAL) << "shape inference failed";
    }

    bool is_input_compressed = strided_ishape.num_elements() < ishape.num_elements();

    helper_.reset(new Conv2DSliceHelper<U64Tensor>(is_input_compressed ? strided_ishape : ishape,
                                                   fshape, slice_strides, paddings));
...
int ConvCoeffIndexCalculator::slice_size(size_t d) const { return helper_->slice_size(d); }

Code ConvCoeffIndexCalculator::Get(std::array<int, 2> coords, TensorShape& out_shape,
...
Code ConvCoeffIndexCalculator::Get(std::array<int, 2> coords, TensorShape& out_shape,
                                   std::vector<size_t>* indices) const {
    ENSURE_OR_RETURN(coords[0] >= 0 && coords[0] < slice_size(1), Code::ERR_INVALID_ARG);
    ENSURE_OR_RETURN(coords[1] >= 0 && coords[1] < slice_size(2), Code::ERR_INVALID_ARG);
    ...
    if (indices) {
        ...
        for (int h = 0; h < out_dims[0]; ++h) {
            for (int w = 0; w < out_dims[1]; ++w) {
                *dst_ptr++ = O + img_indexer(0, h * stride_ + offsets[0], w * stride_ + offsets[1]);
            }
        }
    }
...
Code TensorEncoder::EncodeTensor(const U64Tensor& tensor, const TensorShape& ishape,
                                 const TensorShape& fshape, const Padding& padding,
                                 const size_t& stride, const size_t& n_filters,
                                 const bool is_ntt_form, std::vector<RLWEPt>& enc_tensor) const {

    const size_t N = poly_degree();
    auto out_shape = shape_inference::Conv2D(ishape, fshape, padding, stride).value();
    out_shape.Update(0, n_filters);

    ENSURE_OR_RETURN(out_shape.num_elements() > 0, Code::ERR_INVALID_ARG);
    TensorShape strided_ishape;
    std::array<int, 2> pads{0};
    std::array<int, 3> slice_width{0};
    if (!shape_inference::Conv2D(ishape, fshape, N, padding, stride, strided_ishape, pads,
                                 slice_width)) {
    ...
    ConvCoeffIndexCalculator indexer(N, is_input_compressed ? strided_ishape : ishape, fshape,
                                     is_input_compressed ? Padding::VALID : padding,
                                     is_input_compressed ? 1 : stride);

    const size_t n_one_channel = indexer.slice_size(1) * indexer.slice_size(2);
    enc_tensor.resize(n_filters * n_one_channel);
    if (enc_tensor.size() != n_filters * n_one_channel) {
        return Code::ERR_INTERNAL;
    }

    assert(tensor.shape() == out_shape);
        ...
        for (size_t m = start; m < end; ++m) {
            ...
            for (int sh = 0, hoffset = 0; sh < indexer.slice_size(1); ++sh) {
                for (int sw = 0, woffset = 0; sw < indexer.slice_size(2); ++sw) {
                    ...
                    assert(max <= N);
                    std::vector<U64> coeffs(max + 1, 0);
                    size_t cur = 0;
                    for (long h = 0; h < slice_shape.height(); ++h) {
                        for (long w = 0; w < slice_shape.width(); ++w) {
                            coeffs[indices[cur++]] = tensor(m, hoffset + h, woffset + w);
                        }
                    }
    ...
    return decrypt_program(0, 0, n_filters);
}
