...
class TensorEncoder {
  public:
    ...
    Code EncodeImageShare(Role role, const U64Tensor& img_tensor, const TensorShape& filter_shape,
    ...
    Code EncodeTensor(const U64Tensor& tensor, const TensorShape& ishape, const TensorShape& fshape,
                      const Padding& padding, const size_t& stride, const size_t& n_filters,
                      const bool is_ntt_form, std::vector<RLWEPt>& out) const; // ADDED

  private:
    template <class TensorType, class Indexer>
    Code Encode(TensorShape image_shape, TensorShape filter_shape, const TensorType& img_or_filter,
                const Indexer& indexer, U64* out_poly, size_t out_max_size) const;
    ...
    inline int logq() const { return rt_.first_context_data()->total_coeff_modulus_bit_count(); }

    inline seal::scheme_type scheme() const { return rt_.first_context_data()->parms().scheme(); }
...
class ConvCoeffIndexCalculator {
  public:
    explicit ConvCoeffIndexCalculator(size_t poly_degree, TensorShape ishape, TensorShape fshape,
                                      Padding padding, size_t stride);
    int slice_size(size_t d) const;

    Code Get(std::array<int, 2> coords, TensorShape& out_shape, std::vector<size_t>& indices) const;
