...
class Tensor {
  public:
    ...
    using SignedScalarType =
        typename std::conditional<std::is_integral<ScalarType>::value, int64_t, double>::type;
    ...
    Tensor(const Tensor& oth)
        : shape_(oth.shape_), offsets_(oth.offsets_), raw_data_(oth.raw_data_),
    ...
    static Tensor<ScalarType> Wrap(ScalarType* raw_data, TensorShape shape) {
        ...
        ret.shape_ = shape;
        if (raw_data)
            ret.wrapped_raw_data_ = raw_data;
    ...
    bool IsZero() const {
        ...
        if (0 == NumElements())
            return true;
        return !std::any_of(data(), data() + NumElements(), [](ScalarType c) { return c != 0; });
    }
    ...
    bool IsSameSize(const Tensor& b) const { return shape().IsSameSize(b.shape()); }
    ...
    typename TTypes<ScalarType, 3>::Tensor tensor() {
        ...
        return typename TTypes<ScalarType, 3>::Tensor(data(), shape_.dim_size(0),
                                                      shape_.dim_size(1), shape_.dim_size(2));
    }
    ...
    typename TTypes<ScalarType, 3>::ConstTensor tensor() const {
        ...
        return typename TTypes<ScalarType, 3>::ConstTensor(data(), shape_.dim_size(0),
                                                           shape_.dim_size(1), shape_.dim_size(2));
    }
    ...
    void Randomize(ScalarType range = 1) {
        ...
        if (std::is_same<ScalarType, double>::value) {
            ...
            std::generate_n(wrapped_raw_data_ ? wrapped_raw_data_ : raw_data_.data(),
                            shape_.num_elements(), [&]() { return uniform(rdv_); });
