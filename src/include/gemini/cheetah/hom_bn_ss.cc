...
TensorShape HomBNSS::getSplit(const Meta& meta) const {
    ...
    int64_t sub_vec_len = std::min<int64_t>(meta.vec_shape.length(), poly_degree());
    return TensorShape({nCRT, sub_vec_len});
}

Code HomBNSS::setUp(uint64_t target_base_mod, const seal::SEALContext& context,
                    std::optional<seal::SecretKey> sk, std::shared_ptr<seal::PublicKey> pk) {
    ...
    if (sk) {
        ...
        direct_encryptor_ = std::make_shared<seal::Encryptor>(*direct_context_, *sk);
    }
    ...
    if (pk) {
        ...
        direct_pk_encryptor_ = std::make_shared<seal::Encryptor>(*direct_context_, *pk);
    }
...
Code HomBNSS::setUp(uint64_t target_base_mod, const std::vector<seal::SEALContext>& contexts,
    ...
    for (size_t i = 0; i < nCRT; ++i) {
        ...
        if (!sks.empty()) {
            ENSURE_OR_RETURN(sks[i] && seal::is_valid_for(*sks[i], contexts[i]), Code::ERR_CONFIG);
        }
    ...
    if (!is_mod_2k) {
        if (nCRT != 1 || primes[0].value() != target_base_mod) {
            LOG(FATAL) << "HomBNSS prime field base_mod mismatch " << primes[0].value()
                       << " != " << target_base_mod << " nCRT " << nCRT;
            return Code::ERR_INVALID_ARG;
        }
    } else {
        ...
        if (nbits_crt_plain < 2 * n_base_mod_bits + 1 + kStatBits) {
            LOG(FATAL) << "HomBNSS require more primes to convert the target nbits of "
                          "shares. Require "
                       << 2 * n_base_mod_bits + 1 + kStatBits << " bits of plaintext but got only "
                       << nbits_crt_plain;
            return Code::ERR_INVALID_ARG;
        }
    }
...
Code HomBNSS::encryptVector(const Tensor<uint64_t>& in_vec, const Meta& meta,
    ...
    auto encrypt_prg = [&](long wid, size_t start, size_t end) {
        ...
        for (size_t k = start; k < end; ++k) {
            ...
            out.at(k) = encryptors_[crt_idx]->encrypt_symmetric(enc[k]);
        }
...
Code HomBNSS::encodeVector(const Tensor<uint64_t>& in_vec, const Meta& meta,
                           std::vector<seal::Plaintext>& out, size_t nthreads) const {
...
Code HomBNSS::bn(const std::vector<seal::Ciphertext>& vec_share0,
                 ...
                 size_t nthreads) const {
    ENSURE_OR_RETURN(meta.target_base_mod == target_base_mod_, Code::ERR_OUT_BOUND);
    ENSURE_OR_RETURN(!contexts_.empty() && !evaluators_.empty(), Code::ERR_CONFIG);
    ...
    auto bn_prg = [&](long wid, size_t start, size_t end) {
        for (size_t i = start; i < end; ++i) {
            ...
            if (meta.is_shared_input) {
                evaluators_[crt_idx]->add_plain(vec_share0[i], vec_share1[i], out_share0[i]);
                evaluators_[crt_idx]->multiply_plain_inplace(out_share0[i], scales[i]);
            } else {
                evaluators_[crt_idx]->multiply_plain(vec_share0[i], scales[i], out_share0[i]);
            }
            evaluators_[crt_idx]->mod_switch_to_inplace(out_share0[i],
                                                        contexts_[crt_idx]->last_parms_id());
        }
        return Code::OK;
    };
...
Code HomBNSS::addMaskPrimeField(std::vector<seal::Ciphertext>& cts, Tensor<uint64_t>& mask,
                                const Meta& meta, ThreadPool& tpool) const {
    // TODO Not implemented yet.
    return Code::ERR_INTERNAL;
}

Code HomBNSS::addMaskRing(std::vector<seal::Ciphertext>& cts, Tensor<uint64_t>& mask,
                          const Meta& meta, ThreadPool& tpool) const {
    TensorShape split_shape  = getSplit(meta);
    const size_t nCRT        = split_shape.dim_size(0);
    const size_t sub_vec_len = split_shape.dim_size(1);
    ...
    auto crt_prng = crt_context_->key_context_data()->parms().random_generator()->create();
    ...
    auto mask_prg   = [&](long wid, size_t start, size_t end) {
        ...
        for (size_t i = start; i < end; ++i) {
            ...
            truncate_for_decryption(cts[i], *evaluators_[crt_idx], *contexts_[crt_idx]);
        }
        return Code::OK;
    };
...
Code HomBNSS::addMask(std::vector<seal::Ciphertext>& cts, Tensor<uint64_t>& mask, const Meta& meta,
...
Code HomBNSS::decryptToVector(const std::vector<seal::Ciphertext>& in_vec, const Meta& meta,
                              Tensor<uint64_t>& out_vec, size_t nthreads) const {
    ENSURE_OR_RETURN(meta.target_base_mod == target_base_mod_, Code::ERR_OUT_BOUND);
    ENSURE_OR_RETURN(!contexts_.empty() && !sks_.empty() && !encoders_.empty() && crt_context_,
                     Code::ERR_CONFIG);
    ...
    for (size_t crt = 0; crt < nCRT; ++crt) {
        decryptors[crt] = std::make_shared<seal::Decryptor>(*contexts_.at(crt), *sks_.at(crt));
    }
    ...
    auto decrypt_prg = [&](long wid, size_t start, size_t end) {
        ...
        for (size_t cid = start; cid < end; ++cid) {
            ...
            std::copy_n(slots.cbegin(), vec_pos_end - vec_pos_bgn,
...
Code HomBNSS::encryptTensor(const Tensor<uint64_t>& in_tensor, const Meta& meta,
    ...
    auto encrypt_prg = [&](long wid, size_t start, size_t end) {
        ...
        for (size_t cid = start; cid < end; ++cid) {
            ...
            std::fill(tmp_ptr, tmp.end(), 0);
            auto code = vec2PolyBFV(tmp.data(), tmp.size(), encoded_share[cid], false);
            if (code != Code::OK) {
                LOG(FATAL) << "vec2PolyBFV: " << CodeMessage(code) << std::endl;
            }
            out.at(cid) = direct_encryptor_->encrypt_symmetric(encoded_share[cid]);
        }
...
Code HomBNSS::bn_direct(const std::vector<seal::Ciphertext>& tensor_share0,
    ...
    ENSURE_OR_RETURN(scales.dims() == 1 && scales.length() == meta.ishape.channels(),
                     Code::ERR_CONFIG);
    TensorShape split_shape = getSplitBN(meta.ishape, poly_degree());
    ...
    auto add_prg = [&](long wid, size_t start, size_t end) {
        if (meta.is_shared_input) {
            for (size_t i = start; i < end; ++i) {
                direct_evaluator_->add_plain(tensor_share0[i], tensor_share1[i], out_share0[i]);
            }
        } else {
            for (size_t i = start; i < end; ++i) {
    ...
    auto mul_prog = [&](long wid, size_t start, size_t end) {
        for (size_t c = start; c < end; ++c) {
            ...
            uint64_t v = barrett_reduce_64(static_cast<uint64_t>(std::abs(s)), mod_plain);
            ...
            for (size_t cid = cid_start; cid < cid_end; ++cid) {
                auto cntxt = direct_context_->get_context_data(out_share0[cid].parms_id());
                if (!cntxt) {
                ...
                for (size_t k = 0; k < out_share0[cid].size(); ++k) {
                    ...
                    for (size_t l = 0; l < modulus.size(); ++l) {
                        ...
                        multiply_poly_scalar_coeffmod(rns_ptr, poly_degree(), scalar, modulus[l],
                                                      rns_ptr);
                        rns_ptr += poly_degree();
                    }
                }
    ...
    auto mask_prog = [&](long wid, size_t start, size_t end) {
        ...
        for (size_t cid = start; cid < end; ++cid) {
            direct_evaluator_->mod_switch_to_inplace(out_share0[cid],
                                                     direct_context_->last_parms_id());
            ...
            truncate_for_decryption(out_share0[cid], *direct_evaluator_, *direct_context_);
            seal::util::seal_memzero(rnd[cid].data(), rnd[cid].coeff_count() * sizeof(uint64_t));
        }
        return Code::OK;
    };
    ...
    auto& parms         = direct_context_->get_context_data(out_share0[0].parms_id())->parms();
    ...
    if (IsTwoPower(target_base_mod_)) {
        std::transform(out_share1.data(), out_share1.data() + out_share1.NumElements(),
...
Code HomBNSS::decryptToTensor(const std::vector<seal::Ciphertext>& cts, const Meta& meta,
            ...
            for (int c = 0; c < split_shape.channels(); ++c) {
                for (int h = 0; h < split_shape.height(); ++h) {
                    for (int w = 0; w < split_shape.width(); ++w) {
                        ...
                        out_tensor(offsets[0] + c, offsets[1] + h, offsets[2] + w) = *pt_ptr++;
                    }
                }
            }
        }
