...
class SlicedPaddedTensor {
  public:
    ...
    ScalarType operator()(int c, int h, int w) const {
        ...
        if (c >= shape_.dim_size(0))
            return zero_;

        if (h < hpads_[0] || static_cast<int64_t>(h + hpads_[1]) >= shape_.dim_size(1))
            return zero_;
        if (w < wpads_[0] || static_cast<int64_t>(w + wpads_[1]) >= shape_.dim_size(2))
            return zero_;
    ...
    SlicedPaddedTensor(const SlicedPaddedTensor& oth)
        : zero_(oth.zero_), base_(oth.base_), offsets_(oth.offsets_), hpads_(oth.hpads_),
          wpads_(oth.wpads_), shape_(oth.shape_), mock_shape_(oth.mock_shape_) {}
    ...
    SlicedPaddedTensor(const Base* base, std::array<int, 3> offsets, std::array<int, 2> hpads,
                       std::array<int, 2> wpads, const TensorShape& shape)
        : base_(base), offsets_(offsets), hpads_(hpads), wpads_(wpads), shape_(shape),
          mock_shape_(shape) {
        if (!base_) {
    ...
    SlicedPaddedTensor(const Base* base, std::array<int, 3> offsets, const TensorShape& shape)
    ...
    SlicedPaddedTensor& operator=(const SlicedPaddedTensor& other) = default;
...
class Conv2DSliceHelper {
  public:
    explicit Conv2DSliceHelper(const Base* img, const TensorShape& ishape,
                               const TensorShape& fshape, std::array<int, 3> slice_strides,
                               std::array<int, 2> paddings)
        : base_(img), shape_(ishape),
          padded_shape_({shape_.dim_size(0), shape_.dim_size(1) + paddings[0],
                         shape_.dim_size(2) + paddings[1]}),
          slice_strides_(slice_strides), paddings_(paddings) {
        ...
        if (std::any_of(paddings_.begin(), paddings_.end(), [](int s) { return s < 0; })) {
            LOG(FATAL) << "Conv2DSliceHelper: invalid paddings";
        }
        if (slice_strides_[1] < fshape.dim_size(1) || slice_strides_[2] < fshape.dim_size(1)) {
            LOG(FATAL) << "Conv2DSliceHelper: invalid paddings";
        }
        ...
        for (int d : {1, 2}) {
            ...
            if (padded_shape_.dim_size(d) > slice_strides_[d]) {
                ...
                slices_[d] = 1
                             + CeilDiv<int>(padded_shape_.dim_size(d) - slice_strides_[d],
                                            overlap_strides_[d]);
            } else {
    ...
    Conv2DSliceHelper(const TensorShape& ishape, const TensorShape& fshape,
                      std::array<int, 3> slice_strides, std::array<int, 2> paddings)
    ...
    Code slice(std::array<int, 3> indices, TensorShape& sliced_shape) const {
        ...
        for (int d = 0; d < 3; ++d) {
            ENSURE_OR_RETURN(indices[d] >= 0 && indices[d] < slices_[d], Code::ERR_INVALID_ARG);
            int start = indices[d] * overlap_strides_[d];
            int end   = std::min<int>(start + slice_strides_[d], padded_shape_.dim_size(d));
            sliced_shape.Update(d, end - start);
        }
        return Code::OK;
    }

    Code slice(std::array<int, 3> indices, SlicedPaddedTensor<Base>& sp_tensor) const {
        ENSURE_OR_RETURN(indices.size() == 3, Code::ERR_DIM_MISMATCH);
        ...
        sp_tensor = SlicedPaddedTensor<Base>(base_, clipped_coords, pads[1], pads[2], sliced_shape);
        return Code::OK;
    }
