...
AESNI_ecb_encrypt_blks_ks(block128* blks, unsigned int nblks, const AESNI_KEY* key) {
    ...
    for (unsigned int i = 0; i < nblks; ++i) blks[i] = _mm_xor_si128(one, key[i].rk[0]);
    // blks[i] = _mm_xor_si128(blks[i], key[i].rk[0]);
    for (int j = 1; j < key->rounds; ++j)
        for (unsigned int i = 0; i < nblks; ++i) blks[i] = _mm_aesenc_si128(blks[i], key[i].rk[j]);
...
AESNI_ecb_encrypt_blks(block128* blks, unsigned int nblks, const AESNI_KEY* key) {
    for (unsigned int i = 0; i < nblks; ++i) blks[i] = _mm_xor_si128(blks[i], key->rk[0]);
    for (int j = 1; j < key->rounds; ++j)
        for (unsigned int i = 0; i < nblks; ++i) blks[i] = _mm_aesenc_si128(blks[i], key->rk[j]);
    for (unsigned int i = 0; i < nblks; ++i)
...
static inline void
    ...
    for (j = 1; j < rnds; ++j)
        for (i = 0; i < nblks; ++i) blks[i] = _mm_aesdec_si128(blks[i], key->rk[j]);
    for (i = 0; i < nblks; ++i) blks[i] = _mm_aesdeclast_si128(blks[i], key->rk[j]);
}
